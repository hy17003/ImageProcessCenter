...
unsigned char GeometryTrans::interpolationCube(unsigned char array[4][4], float xpos, float ypos)
{
	...
	for(i=0;i<4;i++){
		t=fabs(col[i]);
		if (t>=0&&t<1)
			col[i]=pow((double)t,3)-2*pow((double)t,2)+1;
		else if (t>=1&&t<2)
			col[i]=-pow((double)t,3)+5*pow((double)t,2)-8*t+4;
		else
			col[i]=0;
	}
	...
	for(i=0;i<4;i++){
		t=fabs(row[i]);
		if (t>=0&&t<1)
			row[i]=pow((double)t,3)-2*pow((double)t,2)+1;
		else if (t>=1&&t<2)
			row[i]=-pow((double)t,3)+5*pow((double)t,2)-8*t+4;
		else
			row[i]=0;
	}
