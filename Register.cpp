...
void Register::SpaceSuppress(ptArray * pCornerpos)
{
	...
	for ( i = 0 ; i < length-1 ; i++) 
	{
		...
		for ( j = i+1 ; j < length ; j++)
		{
			...
			//求取距离
			distacetable[j * length + i] = sqrt((double)pow((double)pt1.x - pt2.x , 2)
				+ pow((double)pt1.y - pt2.y , 2));
	...
	for ( i = 0; i < imWidth ; i++){
		for (j = 0 ; j < imHeight ; j++){
			...
			detOfHessian[j * lineByte + i] 
				...
				- pow((double)fxy[j * lineByte + i] , 2);
			t1=pow((double)mean_array[j*lineByte+i] , 2);
			t2=detOfHessian[j * lineByte + i];
			norm_array[j*lineByte+i] = sqrt((double)t1-t2);
	...
	for ( i = 0 ; i < cnt1 ; i++)
	{
		for ( j = 0 ; j < cnt2 ; j++)
		{
			...
			for ( m = - (int) (W/2) ; m <= (int) (W/2) ; m++ )
			{
				for ( n = -(int) (W/2) ; n <= (int) (W/2) ; n++ )
				{
					sigama1 += pow((double)*(imgBuf1+lineByte*(y1+m)+x1+n)-aver1,2);
					sigama2 += pow((double)*(imgBuf2+lineByte*(y2+m)+x2+n)-aver2,2);
					temp += (*(imgBuf1+lineByte*(y1+m)+x1+n)-aver1)
						*(*(imgBuf2+lineByte*(y2+m)+x2+n)-aver2);
				}
			}
			sigama1=sqrt((double)double(sigama1));
            sigama2=sqrt((double)double(sigama2));
            Cij=temp/(W*W*sigama1*sigama2);
            r=pow((double)x1-x2,2)+pow((double)y1-y2,2);
...
{ 
	...
	{ 
		d=sqrt((double)fg[0]*fg[0]+fg[1]*fg[1]);
